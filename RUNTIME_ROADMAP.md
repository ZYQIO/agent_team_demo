...
- Improve practical Claude-style feature parity in a way that stays usable inside Codex
- Keep CLI, tests, artifacts, and skill scripts stable during the refactor

Benchmark review (2026-03-11):
...
1. Upgrade the current lead-facing interaction plus preview-capable plan approval workflow into a richer embedded in-run control surface
   Goal: build past live snapshots, teammate session summaries, teammate detail inspection, teammate review output, pending-teammate review summaries, next-pending review guidance, current-review-focus actions, current-focus teammate follow-on requests, teammate-scoped approval actions, file-backed commands, the terminal lead console, and the embedded stdin approval prompt toward a more coherent Codex-usable interaction model.
