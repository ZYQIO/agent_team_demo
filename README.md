...
Host mode currently uses external session-worker subprocesses for mailbox-driven reviewer flows (`peer_challenge`, `evidence_pack`), reviewer `llm_synthesis`, and teammate auto-replies, while host planning/report tasks still execute on the lead-managed inline path.
