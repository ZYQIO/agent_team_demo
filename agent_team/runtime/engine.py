...
def request_teammate_statuses(
    ...
    for raw_name in agent_names or []:
        ...
        lead_context.mailbox.send(
            ...
            subject=LEAD_STATUS_REQUEST_SUBJECT,
            body=json.dumps(
                {
        ...
        lead_context.logger.log(
            "lead_status_request_sent",
            agent=agent_name,
            decision_source=decision_source,
        )
        requested.append(agent_name)
    return {
        "status_request_agents": requested,
        "invalid_status_request_agents": invalid,
    }
...
def parse_interactive_plan_command(raw_command: str) -> Dict[str, Any]:
    ...
    if len(parts) == 2 and parts[1].strip().lower() == "current":
        ...
        if verb in {"approve", "approve-plan"}:
            return {"action": "approve_current_review", "raw": text, "task_id": ""}
        if verb in {"reject", "reject-plan"}:
            return {"action": "reject_current_review", "raw": text, "task_id": ""}
        if verb in {"status", "request-status", "request_status"}:
            return {"action": "request_current_review_status", "raw": text, "task_id": ""}
        if verb in {"plan", "request-plan", "request_plan"}:
            return {"action": "request_current_review_plan", "raw": text, "task_id": ""}
...
def _describe_pending_teammate_reviews(interaction_snapshot: Mapping[str, Any]) -> List[str]:
    pending_requests = [
        item
        for item in interaction_snapshot.get("plan_approval_requests", [])
        if isinstance(item, Mapping)
...
def _print_interactive_plan_approval_status(
    ...
    if str(current_review_agent or ""):
        print(
            f"[lead] current_review_focus={str(current_review_agent or '')} use `status current`, `plan current`, `approve current`, or `reject current`",
            flush=True,
        )
    ...
    print(
        "[lead] commands: approve <task_id> | reject <task_id> | approve teammate <agent> | reject teammate <agent> | approve current | reject current | approve-all | show | show <task_id> | teammate <agent> | show teammate <agent> | review pending | review next | review teammate <agent> | status <agent> | plan <agent> | status current | plan current | pause",
        flush=True,
    )
...
def run_interactive_plan_approval_prompt(
    ...
    while pending_plan_approvals:
        ...
        _print_interactive_plan_approval_status(
            ...
            current_review_agent=current_review_agent,
        )
        try:
        ...
        if str(raw_command or "").strip():
            record_lead_command(
                ...
                agent=(
                    task_id
                    if action in {"request_teammate_status", "request_teammate_plan", "show_teammate", "review_teammate", "approve_teammate_plans", "reject_teammate_plans"}
                    else current_review_agent
                    if action in {"approve_current_review", "reject_current_review", "request_current_review_status", "request_current_review_plan"}
                    else ""
                ),
        ...
        if action == "review_next":
            ...
            if not next_pending_agent:
                ...
                continue
            current_review_agent = next_pending_agent
        ...
        if action == "help":
            print(
                "[lead] help: approve <task_id> | reject <task_id> | approve teammate <agent> | reject teammate <agent> | approve current | reject current | approve-all | show | show <task_id> | teammate <agent> | show teammate <agent> | review pending | review next | review teammate <agent> | status <agent> | plan <agent> | status current | plan current | pause",
                flush=True,
            )
            continue
        if action in {"request_current_review_plan", "request_current_review_status"}:
            if not current_review_agent:
                print("[lead] no current review focus. Use `review next` or `review teammate <agent>` first.", flush=True)
                continue
            if action == "request_current_review_plan":
                plan_resolution = request_teammate_plans(
                    lead_context=lead_context,
                    agent_names=[current_review_agent],
                ...
                if plan_resolution.get("plan_request_agents"):
                    ...
                    print(f"[lead] unknown teammate: {current_review_agent}", flush=True)
                continue
            status_resolution = request_teammate_statuses(
                lead_context=lead_context,
                agent_names=[current_review_agent],
            ...
            if status_resolution.get("status_request_agents"):
                print(
                    "[lead] requested teammate status from "
                    + ", ".join(status_resolution.get("status_request_agents", [])),
                    flush=True,
                )
            else:
                print(f"[lead] unknown teammate: {current_review_agent}", flush=True)
        ...
        if action == "invalid":
            ...
            continue
        if action in {"approve_current_review", "reject_current_review"}:
            if not current_review_agent:
                print("[lead] no current review focus. Use `review next` or `review teammate <agent>` first.", flush=True)
                continue
            request_resolution = resolve_pending_plan_request_task_ids(
                lead_context=lead_context,
                requested_by_agents=[current_review_agent],
            )
            resolved_task_ids = list(request_resolution.get("task_ids", []))
            if not resolved_task_ids:
                print(f"[lead] no pending approvals requested by {current_review_agent}", flush=True)
                current_review_agent = ""
                continue
            resolution = apply_requested_plan_approvals(
                lead_context=lead_context,
                approve_task_ids=resolved_task_ids if action == "approve_current_review" else [],
                reject_task_ids=resolved_task_ids if action == "reject_current_review" else [],
                decision_source="interactive",
            )
            print(
                "[lead] "
                + (
                    f"approved current review focus {current_review_agent}: "
                    if action == "approve_current_review"
                    else f"rejected current review focus {current_review_agent}: "
                )
            ...
            current_review_agent = _next_pending_review_agent_name(
                interaction_snapshot=refresh_live_lead_interaction(lead_context=lead_context).get("snapshot", {})
            )
            if not pending_plan_approvals:
                return {
                    **resolution,
        ...
        if action in {"approve_teammate_plans", "reject_teammate_plans"}:
            ...
            if resolution.get("pending_task_ids"):
                ...
                if task_id == current_review_agent:
                    current_review_agent = _next_pending_review_agent_name(
                        interaction_snapshot=refresh_live_lead_interaction(lead_context=lead_context).get("snapshot", {})
                    )
                continue
