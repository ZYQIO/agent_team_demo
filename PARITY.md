...
This file tracks practical parity against Claude Code Agent Teams as of 2026-03-17.
...
| Lead-facing team interaction surface | Partial | Runtime now emits live-updating `lead_interaction.json` + `lead_interaction.md` snapshots with centralized lead-visible mailbox traffic and pending teammate plan approvals, including previews of proposed inserted tasks and dependency additions, can consume live file-backed lead commands from `lead_commands.jsonl` while waiting on approval, ships a terminal `lead_console.py` helper for in-run inspection plus `show <task_id>` / approve / reject commands, and can prompt on stdin with `--lead-interactive` when approval is pending, but it still lacks a richer embedded in-run UI. |
...
| Explicit session boundary posture | Implemented | Runtime now emits `host_enforcement.json` plus `session_boundaries.json`, distinguishes the active host session backend from advertised host capabilities, records when host-mode runs are still transport-backed through `host_session_backend=external_process`, and preserves host-managed session posture for real backends such as `host_session_backend=codex_exec`. |
...
| Teammate mode `host` | Partial | `--teammate-mode host` now executes teammate tasks through a distinct host transport path, gives host/helper contexts transport-local mailbox views, dispatches analyst scans/follow-ups plus reviewer mailbox/planning/report/llm tasks through explicit `session_task_assignment` mailbox messages, applies teammate result/state/task completion and task-graph mutations through explicit `session_task_result` mailbox messages on the lead side, and routes host session-ledger updates through explicit `session_telemetry` mailbox messages. `host_kind=codex` now uses a true host-backed `codex_exec` session backend that reuses persisted Codex thread ids, while `host_kind=claude-code` still remains on the transport-backed `external_process` worker path in this environment. |
...
| Team plan approval workflow | Partial | When policy `teammate_plan_required` is enabled, teammate-generated task mutations are queued as pending approvals with previews of proposed task/dependency changes, lead can either resume with `--approve-plan`, `--reject-plan`, or `--approve-all-pending-plans`, keep the run alive and feed live file-backed commands through `lead_commands.jsonl`, use the terminal `lead_console.py` helper against the live interaction snapshot, or stay inside the runtime process with `--lead-interactive`; both live control surfaces now support `show <task_id>` for detailed inspection of one pending request, but there is not yet a richer embedded in-run interactive UI. |
...
| Real independent LLM teammate sessions | Partial | Runtime now maintains a durable per-agent teammate session ledger with stable session ids across resume, transport, recent task history, provider memory, explicit `teammate_session_initialized`/`teammate_session_resumed` lifecycle events, transport-local mailbox views for worker/helper contexts, and host-mode teammate assignment/result/telemetry contracts. The repo now has one true host-backed session backend through persisted Codex sessions (`codex_exec`), but Claude-parity host execution is still partial because the `claude-code` host kind is not yet backed by a trustworthy native session backend in this environment. |

## Direction Check

- No major drift in the core objective: the repository is still aiming at the right Claude Code Agent Teams shape.
- Remaining drift is narrower now: lead-facing interaction and plan approval exist as live snapshots + resume/CLI + file-backed live-command + terminal-console + embedded-stdin workflows with detailed per-request inspection, and the runtime now has one true host-backed session backend via Codex, but the repo still trails Claude Code on richer embedded in-run interaction and on a trustworthy `claude-code` host backend.

## Near-Term Focus

1. Replace the remaining `claude-code` `external_process` session-worker backend with a trustworthy true host-backed teammate session while preserving the explicit mailbox/result/telemetry contracts.
2. Upgrade lead-facing interaction plus plan approval from live snapshots plus terminal/file-backed/embedded-stdin command workflow into a richer embedded in-run control surface.
