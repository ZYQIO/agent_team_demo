...
| Real independent LLM teammate sessions | Partial | Runtime now maintains a durable per-agent teammate session ledger with stable session ids across resume, transport, recent task history, provider memory, explicit `teammate_session_initialized`/`teammate_session_resumed` lifecycle events, transport-local mailbox views for worker/helper contexts, and host-mode mailbox reviewer/session-ledger assignment/result/telemetry contracts onto long-lived session threads; host mode records host-managed boundaries through a distinct transport path, but true isolated external process/LLM sessions are not universal yet. |
