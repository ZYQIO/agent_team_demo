...
| Explicit session boundary posture | Partial | Runtime now emits `session_boundaries.json` to classify each teammate session as host-native, tmux-backed, worker-subprocess-backed, or runtime-emulated, and tmux-mode workers now get session-scoped workspace/temp directories; host-level enforcement is still not universal. |
