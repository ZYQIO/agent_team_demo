...
| Teammate mode `host` | Partial | `--teammate-mode host` now executes teammate tasks through a distinct host transport path, records `host_native_session` / `host_native_workspace` boundaries, emits `claude-code:<agent>` transport session names plus `host://...` workspace roots, and gives host/helper contexts transport-local mailbox views, but it still does not launch true external host-backed teammate sessions. |
...
| Real independent LLM teammate sessions | Partial | Runtime now maintains a durable per-agent teammate session ledger with stable session ids across resume, transport, recent task history, provider memory, explicit `teammate_session_initialized`/`teammate_session_resumed` lifecycle events, and transport-local mailbox views for worker/helper contexts; host mode records host-managed boundaries through a distinct transport path, but true isolated external process/LLM sessions are not universal yet. |
