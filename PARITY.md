...
| Real independent LLM teammate sessions | Partial | Runtime now maintains a durable per-agent teammate session ledger with stable session ids across resume, transport, recent task history, provider memory, explicit `teammate_session_initialized`/`teammate_session_resumed` lifecycle events, transport-local mailbox views for worker/helper contexts, and host-mode mailbox reviewer dispatch through explicit assignment messages onto long-lived session threads; host mode records host-managed boundaries through a distinct transport path, but true isolated external process/LLM sessions are not universal yet. |
...
1. Reassess mailbox-driven reviewer tasks (`peer_challenge`, `evidence_pack`) for isolation boundaries without breaking mailbox semantics.
2. Expand the executable `host` transport skeleton toward true external host-backed teammate sessions after the mailbox boundary is defined.
