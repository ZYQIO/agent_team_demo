...
| Real independent LLM teammate sessions | Partial | Runtime now maintains a durable per-agent teammate session ledger with session ids, transport, recent task history, and provider memory, but true isolated process/LLM sessions are not universal yet. |
