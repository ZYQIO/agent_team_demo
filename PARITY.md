...
| Team plan approval workflow | Partial | When policy `teammate_plan_required` is enabled, teammate-generated task mutations are queued as pending approvals, lead can either resume with `--approve-plan`, `--reject-plan`, or `--approve-all-pending-plans`, keep the run alive and feed live file-backed commands through `lead_commands.jsonl`, use the terminal `lead_console.py` helper against the live interaction snapshot, or stay inside the runtime process with `--lead-interactive`; there is not yet a richer embedded in-run interactive UI. |
