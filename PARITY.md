...
| Team plan approval workflow | Partial | When policy `teammate_plan_required` is enabled, teammate-generated task mutations are queued as pending approvals, lead can either resume with `--approve-plan`, `--reject-plan`, or `--approve-all-pending-plans`, keep the run alive and feed live file-backed commands through `lead_commands.jsonl`, or use the terminal `lead_console.py` helper against the live interaction snapshot; there is not yet a richer embedded in-run interactive UI. |
...
- Remaining drift is narrower now: lead-facing interaction and plan approval exist as live snapshots + resume/CLI + file-backed live-command + terminal-console workflows, but the repo still trails Claude Code on richer embedded in-run interaction and true host-backed teammate sessions.
...
2. Upgrade lead-facing interaction plus plan approval from live snapshots plus terminal/file-backed command workflow into a richer embedded in-run control surface.
