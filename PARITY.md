...
| Teammate mode `host` | Partial | `--teammate-mode host` now executes teammate tasks through a distinct host transport path, records `host_native_session` / `host_native_workspace` boundaries, emits `claude-code:<agent>` transport session names plus `host://...` workspace roots, gives host/helper contexts transport-local mailbox views, dispatches mailbox-driven reviewer tasks through explicit `session_task_assignment` mailbox messages onto long-lived session threads, and applies reviewer result/state/task completion through explicit `session_task_result` mailbox messages on the lead side, but it still does not launch true external host-backed teammate sessions. |
...
| Real independent LLM teammate sessions | Partial | Runtime now maintains a durable per-agent teammate session ledger with stable session ids across resume, transport, recent task history, provider memory, explicit `teammate_session_initialized`/`teammate_session_resumed` lifecycle events, transport-local mailbox views for worker/helper contexts, and host-mode mailbox reviewer assignment/result contracts onto long-lived session threads; host mode records host-managed boundaries through a distinct transport path, but true isolated external process/LLM sessions are not universal yet. |
...
1. Expand the executable `host` transport skeleton toward true external host-backed teammate sessions now that mailbox reviewer assignment/result contracts exist.
2. Reassess whether any remaining reviewer mailbox/session lifecycle state still crosses in-process objects after the external boundary lands.
3. Add a lead-facing team interaction surface plus plan approval for teammate-driven task mutations.
4. Add true event-level state replay (not only event-index-to-checkpoint mapping).
