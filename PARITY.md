...
| Explicit session boundary posture | Partial | Runtime now emits `host_enforcement.json` plus `session_boundaries.json`, so advertised host capabilities are separated from runtime-active host-native isolation; tmux-mode workers still provide the only broadly active strong boundary path today. |
...
| Teammate mode `subprocess` | Partial | Analyst task execution can now run as dedicated worker subprocesses with session-scoped workdir/home/target snapshots and `worker_subprocess_session` boundaries, while reviewer tasks still run in-process. |
