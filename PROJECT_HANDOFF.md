...
- Stable capabilities:
  ...
  - live lead interaction snapshots plus preview-capable teammate plan approval through resume CLI, file-backed commands, `lead_console.py`, and embedded stdin prompt, with `show <task_id>` detail inspection and live teammate status requests (`status <agent>`) in the live console / command surfaces
