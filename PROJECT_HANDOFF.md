...
- Stable capabilities:
  ...
  - live lead interaction snapshots plus preview-capable teammate plan approval through resume CLI, file-backed commands, `lead_console.py`, and embedded stdin prompt, with teammate session summaries, `show <task_id>` detail inspection, teammate detail inspection, combined teammate review output, pending-teammate review summaries, next-pending review hints, current embedded review focus for `status current` / `plan current` / `approve current` / `reject current`, live teammate status/plan requests (`status <agent>`, `plan <agent>`), and teammate-scoped plan approval commands in the live console / command surfaces
...
1. Lead-facing team interaction is still not a richer embedded in-run surface.
   Plan approval now exists as live snapshots plus CLI/file-backed/terminal/embedded-stdin controls, live control surfaces can inspect one pending request in detail with `show <task_id>`, inspect one teammate in detail with teammate/session commands, run a combined teammate review, scan pending approvals by teammate, jump to the next pending teammate review, keep an embedded current review focus, see teammate session summaries, request teammate status/plan replies during the run including against the current focus, and approve/reject pending plans by teammate identity, but it still needs a more unified Codex-friendly control loop.
...
- Lead can now see teammate session summaries, inspect a specific teammate in detail, run a combined teammate review, and request status/plan replies during a blocked approval window, so the next high-value step is unifying those pieces into a more embedded control surface instead of adding more one-off command verbs.
